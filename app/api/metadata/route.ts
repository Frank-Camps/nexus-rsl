...
export async function POST(request: Request) {
    try {
        ...
        const newItem = await Metadata.create(body);
...
export async function PUT(request: Request) {
    try {
        ...
        const updatedItem = await Metadata.findByIdAndUpdate(
            id,
            updateData,
...
export async function DELETE(request: Request) {
    try {
        ...
        const deletedItem = await Metadata.findByIdAndDelete(id);
        ...
        console.error("Erreur DELETE Metadata:", error);
        return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
    }
}